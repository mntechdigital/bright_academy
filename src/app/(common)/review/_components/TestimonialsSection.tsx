"use client";
import React, { useState } from "react";

const testimonials = [
  {
    name: "সাকিব হাসান",
    role: "শিক্ষার্থী - ক্লাস ১০",
    quote:
      "এখানকার শিক্ষা পদ্ধতি অনেক আধুনিক। গণিত আর বিজ্ঞানে আমার দুর্বলতা কাটিয়ে উঠতে পেরেছি। এখন আমি ক্লাসে প্রথম হই!",
  },
  {
    name: "তানিয়া আক্তার",
    role: "অভিভাবক",
    quote:
      "আমার মেয়ের পড়াশোনায় অসাধারণ উন্নতি হয়েছে। শিক্ষকরা অনেক যত্নশীল এবং প্রতিটি শিক্ষার্থীর দিকে মনোযোগ দেন।",
  },
  {
    name: "রাহুল দেব",
    role: "শিক্ষার্থী - ক্লাস ৮",
    quote:
      "কম্পিউটার ল্যাব এবং আধুনিক সুবিধা পেয়ে পড়াশোনা আরও আনন্দময় হয়েছে। এখানে পড়তে আসা আমার জীবনের সেরা সিদ্ধান্ত।",
  },
];

const TestimonialsSection = () => {
  const [active, setActive] = useState(0);
  const t = testimonials[active];

  return (
    <section className="w-full bg-[#fdf8f3] overflow-hidden relative py-14 px-6">

      {/* Top-left orange blob */}
      <div className="absolute -top-6 -left-6 w-20 h-20 bg-orange-400 rounded-full opacity-80 pointer-events-none" />

      {/* Bottom-right orange arc */}
      <div className="absolute -bottom-8 -right-4 w-20 h-20 border-14 border-orange-400 rounded-full opacity-80 pointer-events-none" />

      <div className="max-w-5xl mx-auto flex flex-col md:flex-row items-center gap-10">

        {/* Left: Avatar with quote icon */}
        <div className="relative shrink-0">
          <div className="w-48 h-48 rounded-full bg-gray-200 flex items-center justify-center overflow-hidden">
            <svg xmlns="http://www.w3.org/2000/svg" width="90" height="90" viewBox="0 0 24 24" fill="#9ca3af">
              <path d="M12 12c2.7 0 4.8-2.1 4.8-4.8S14.7 2.4 12 2.4 7.2 4.5 7.2 7.2 9.3 12 12 12zm0 2.4c-3.2 0-9.6 1.6-9.6 4.8v2.4h19.2v-2.4c0-3.2-6.4-4.8-9.6-4.8z"/>
            </svg>
          </div>
          {/* Quote mark badge */}
          <div className="absolute -left-4 bottom-6 w-10 h-10 bg-white rounded-full shadow flex items-center justify-center">
            <span className="text-gray-700 text-xl font-serif leading-none" style={{ marginTop: "-4px" }}>"</span>
          </div>
        </div>

        {/* Right: Content */}
        <div className="flex-1">
          {/* Eyebrow */}
          <div className="inline-flex items-center border border-gray-300 rounded px-3 py-1 mb-4">
            <span className="text-xs text-gray-500 tracking-wide">শিক্ষার্থীদের মতামত</span>
          </div>

          {/* Heading */}
          <h2 className="text-2xl md:text-3xl font-bold text-gray-900 mb-4 leading-snug">
            আমাদের সফল শিক্ষার্থী ও অভিভাবকদের অভিজ্ঞতা
          </h2>

          {/* Quote */}
          <p className="text-sm text-gray-600 leading-relaxed mb-5 max-w-lg">
            "{t.quote}"
          </p>

          {/* Name */}
          <p className="text-sm font-bold text-gray-900 mb-4">
            {t.name}{" "}
            <span className="font-normal text-gray-400">— {t.role}</span>
          </p>

          {/* Dots */}
          <div className="flex items-center gap-2">
            {testimonials.map((_, i) => (
              <button
                key={i}
                onClick={() => setActive(i)}
                className={`h-2 rounded-full transition-all duration-300 ${
                  i === active
                    ? "w-8 bg-orange-400"
                    : "w-4 bg-gray-300 hover:bg-gray-400"
                }`}
                aria-label={`Testimonial ${i + 1}`}
              />
            ))}
          </div>
        </div>
      </div>
    </section>
  );
};

export default TestimonialsSection;