...
import { deleteClass } from "@/src/services/classes";
import { showErrorToast, showSuccessToast } from "@/src/utils/toastMessage";
import { deleteMonthlyResult } from "@/src/services/monthlyResult";

const DeleteMonthlyResultDialog = ({ id }: { id?: string }) => {
  const [isOpen, setIsOpen] = useState(false);
  const [isPending, startTransition] = useTransition();

  const handleDelete = async () => {
    startTransition(async () => {
      const result = await deleteMonthlyResult(id);
...
export default DeleteMonthlyResultDialog;
